...
export async function uploadFileToSupabaseStorage(
  ...
  try {
    ...
    if (error) {
        ...
        try {
          ...
          if (!flatRes.error && flatRes.data) {
            ...
            if (pubData?.publicUrl) {
              console.log('[directSupabaseStorage] Flat path upload succeeded:', pubData.publicUrl);
      ...
      // 2. Fallback to alternative bucket if target bucket has permissions or config mismatch
      const altBucket = targetBucket === 'products' ? 'avatars' : 'products';
      try {
        ...
        if (!fallbackRes.error && fallbackRes.data) {
          ...
          if (pubData?.publicUrl) {
            console.log('[directSupabaseStorage] Fallback bucket upload succeeded:', pubData.publicUrl);
            return pubData.publicUrl;
          }
        }
      } catch (altErr) {
        console.warn('[directSupabaseStorage] Alt bucket attempt failed:', altErr);
      }

      // 3. Fallback via server-side /api/upload endpoint to guarantee permanent cloud/server URL
      try {
        let base64Body = '';
        ...
        console.warn('[directSupabaseStorage] Server upload fallback error:', serverErr);
      }

      // 4. Construct permanent Supabase public URL without embedding raw base64
    ...
    // Never return raw Base64 data strings into image_url
