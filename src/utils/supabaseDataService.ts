...
export function prepareProductPayload(raw: Record<string, any>): Record<string, any> {
  ...
  const sku = String(raw.sku || raw.code || raw.product_code || '').trim();
  ...
  const payload: Record<string, any> = {
    // Exact columns present in Supabase 'products' table across schema versions
    name: name,
    product_name: name,
    products_name: name,
    name_bn: name,
    ...
    unit: unit,
    unit_pack: unit,
    unit_type: raw.unit_type || raw.unitType || undefined,
    stock: stock,
