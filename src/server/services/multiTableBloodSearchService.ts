...
export async function verifyUserRegistration(rawMobile: string): Promise<{
  ...
  // 1. Check all 4 Supabase registration tables simultaneously without requiring login:
  // - product_sellers
  // - service_providers
  // - permanent_members
  // - blood_donors
  try {
    const [sellerRes, proRes, memberRes, donorRes] = await Promise.all([
    ...
    if (!donorRes.error && donorRes.data && donorRes.data.length > 0) {
      ...
      return {
        ...
        matchedPhone: match.phone_number || cleanMobile,
  ...
  try {
    const [allDonors, allSellers, allPros, allMembers] = await Promise.all([
      supabase.from('blood_donors').select('id, full_name, phone_number, whatsapp_number').limit(200),
      supabase.from('product_sellers').select('id, shop_or_owner_name, phone_number, whatsapp_number').limit(200),
      supabase.from('service_providers').select('id, full_name, phone_number, whatsapp_number').limit(200),
      supabase.from('permanent_members').select('id, full_name, phone_number, whatsapp_number').limit(200),
    ]);

    const dMatch = allDonors.data?.find((d: any) => phonesMatch(d.phone_number, cleanMobile) || phonesMatch(d.whatsapp_number, cleanMobile));
    if (dMatch) {
      return {
        isRegistered: true,
        matchedTable: 'blood_donors',
        matchedName: dMatch.full_name || 'রক্তদাতা',
        matchedPhone: dMatch.phone_number || cleanMobile,
        message: 'মোবাইল নম্বরটি নিবন্ধিত পাওয়া গেছে।'
      };
    }

    const sMatch = allSellers.data?.find((s: any) => phonesMatch(s.phone_number, cleanMobile) || phonesMatch(s.whatsapp_number, cleanMobile));
    if (sMatch) {
      return {
        isRegistered: true,
        matchedTable: 'product_sellers',
        matchedName: sMatch.shop_or_owner_name || 'পণ্য বিক্রেতা',
        matchedPhone: sMatch.phone_number || cleanMobile,
        message: 'মোবাইল নম্বরটি নিবন্ধিত পাওয়া গেছে।'
      };
    }

    const pMatch = allPros.data?.find((p: any) => phonesMatch(p.phone_number, cleanMobile) || phonesMatch(p.whatsapp_number, cleanMobile));
    if (pMatch) {
      return {
        isRegistered: true,
        matchedTable: 'service_providers',
        matchedName: pMatch.full_name || 'সেবাদাতা',
        matchedPhone: pMatch.phone_number || cleanMobile,
        message: 'মোবাইল নম্বরটি নিবন্ধিত পাওয়া গেছে।'
      };
    }

    const mMatch = allMembers.data?.find((m: any) => phonesMatch(m.phone_number, cleanMobile) || phonesMatch(m.whatsapp_number, cleanMobile));
    if (mMatch) {
      return {
        isRegistered: true,
        matchedTable: 'permanent_members',
        matchedName: mMatch.full_name || 'স্থায়ী সদস্য',
        matchedPhone: mMatch.phone_number || cleanMobile,
        message: 'মোবাইল নম্বরটি নিবন্ধিত পাওয়া গেছে।'
      };
    }
  } catch (err) {
  ...
  // Not found in any of the 4 registration tables
  return {
    isRegistered: false,
    message: 'আপনার মোবাইল নম্বরটি রেজিস্ট্রেশন করা নেই, দয়া করে রেজিস্ট্রেশন করুন'
  };
