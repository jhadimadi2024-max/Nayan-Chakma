...
export const ProductDetailsScreen: React.FC<ProductDetailsScreenProps> = ({
  ...
  // Dynamic Quantity Steps based on product.unit_type (250g, 1kg, piece)
  const quantitySteps = useMemo(() => {
    return generateQuantitySteps(product);
  }, [product]);

  const [selectedStepIndex, setSelectedStepIndex] = useState(0);

  // Reset to initial step whenever product changes
  useEffect(() => {
    setSelectedStepIndex(0);
  }, [product.id]);

  const currentStep: ProductQuantityStep = quantitySteps[selectedStepIndex] || quantitySteps[0] || {
    index: 0,
    value: 1,
    labelBn: '১ টি',
    labelEn: '1 pc',
  ...
  const discountPercent = (hasGenuineDiscount && rawOriginal && rawDiscount) 
    ...
    : 0;
  const dynamicTotalPrice = calculateDynamicStepPrice(unitPrice, currentStep);
  const dynamicOriginalTotalPrice = calculateDynamicStepPrice(originalPrice, currentStep);
  ...
  const handleQuantityIncrement = () => {
    setSelectedStepIndex(prev => Math.min(prev + 1, quantitySteps.length - 1));
  };

  const handleQuantityDecrement = () => {
    setSelectedStepIndex(prev => Math.max(prev - 1, 0));
  };
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-8 pb-28 sm:pb-16">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-5">
            ...
            >
              {/* পরিমাণ নির্বাচন (Dynamic Quantity Selector based on product.unit_type) */}
              <div className="bg-slate-50 p-3.5 rounded-2xl border border-gray-200 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500 font-medium">
                      বর্তমান ধাপ: <strong className="text-emerald-700">{currentStep.labelBn}</strong> (স্টকে আছে: {product.stock_quantity ?? product.stockQuantity ?? 99} টি)
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 bg-white px-2 py-1 rounded-xl border border-gray-300 shadow-2xs">
                    ...
                    <button
                      ...
                      disabled={selectedStepIndex >= quantitySteps.length - 1}
                ...
                {/* Quick Step Selection Pills */}
                {quantitySteps.length > 1 && (
                  <div className="pt-2 border-t border-gray-200/80">
                    <div className="flex items-center justify-between text-[11px] mb-1.5">
                      <span className="text-slate-500 font-medium">ধাপ নির্বাচন করুন (Select Step):</span>
                      <span className="text-emerald-700 font-bold font-mono text-[10px]">
                        {currentStep.labelBn}
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {quantitySteps.slice(0, 8).map((step, idx) => {
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                              isSelected
                                ...
                                : 'bg-white hover:bg-slate-200 text-slate-700 border border-gray-200'
                            }`}
                          >
                            {step.shortLabel}
                          </button>
                ...
                <div className="text-right">
                  ...
                  {currentStep.multiplier !== 1 && (
                    <span className="text-[10px] text-slate-500 block font-normal">
                      (৳ {unitPrice.toLocaleString('bn-BD')} / {product.unit_pack || product.unit || 'একক'} × {currentStep.shortLabel})
                    </span>
                  )}
                </div>
              </div>
                ...
                <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-gray-200/70">
                  ...
                  <div className="text-right">
                    <span className="text-xs font-black text-[#16a34a] block">
                      মোট: ৳ {dynamicTotalPrice.toLocaleString('bn-BD')}
                    </span>
                    {currentStep.multiplier !== 1 && (
                      <span className="text-[9px] text-slate-400 font-mono block">
                        (৳ {unitPrice} × {currentStep.shortLabel})
                      </span>
                    )}
                  </div>
                </div>
